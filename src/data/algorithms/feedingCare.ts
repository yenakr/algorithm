...
export const feedingCareAlgorithm = {
  ...
  questions: {
    ...
    q3: {
      ...
      resultId: (answers: Record<string, any>) => {
        ...
        if (val === 'yes') return 'F-B';
        if (val === 'no') return 'F-C';
        return null;
      }
    } as Question,
  },
  results: {
    ...
    'F-B': {
      ...
      reason: '팔 근력이 3등급 미만으로 스스로 팔을 들지 못해 숟가락질이 아예 불가능하므로 입 위치로 음식을 전달하는 전자동 식사로봇을 권장합니다.',
      simpleResultSummary: '상지 근력 상실로 식사 동작은 불가능하지만 고개 조작 및 구강 섭취가 가능하므로 전자동 식사보조 로봇을 추천합니다.'
    } as Result,
    'F-C': {
      id: 'F-C',
      title: '팔의 근력에 따라,\n• 부분 식사보조기기 이용 (예: 스프링 팔 지지대)\n• 수동/반자동 식사돌봄로봇 이용',
      simpleTitle: '팔의 근력에 따라,\n• 부분 식사보조기기 이용 (예: 스프링 팔 지지대)\n• 수동/반자동 식사돌봄로봇 이용',
      description: '팔의 근력에 따라,\n• 부분 식사보조기기 이용 (예: 스프링 팔 지지대)\n• 수동/반자동 식사돌봄로봇 이용',
      simpleDescription: '팔의 근력에 따라,\n• 부분 식사보조기기 이용 (예: 스프링 팔 지지대)\n• 수동/반자동 식사돌봄로봇 이용',
      ...
      reason: '팔을 어느 정도 스스로 움직일 수 있으므로(3등급 이상), 환자의 잔존 동작 역량을 보조하고 피로를 줄여주는 부분식사보조나 반자동 기기가 추천됩니다.',
    ...
    'F-D': {
      ...
      recommendation: '모든 구강 식이를 배제하고 수액이나 튜브를 이용해 안전을 도모하고 주치와의 연하 치료를 상담하십시오.',
      simpleRecommendation: '안전을 위해 입으로 드시지 마시고, 튜브나 수액을 통한 영양 공급 관리에 집중하며 사래 기침 발생 시 즉시 식사를 중단하고 의료진을 찾으십시오.',
      reason: '삼킴이 전적으로 원활하지 않아 음식물 유입에 의한 질식 위험이 극대화되어 있으므로 비구강식 임상 영양 프로토콜을 따라야 합니다.',
