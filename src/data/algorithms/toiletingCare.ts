...
export const toiletingCareAlgorithm = {
  ...
  questions: {
    ...
    // 경로 A: 인지 어려움이 낮은 경우 (0~1점)
    q2_a: {
      id: 'q2_a',
      title: 'Q2-A. 화장실 이동 평가 (경로 A)',
    ...
    q3_a1: {
      id: 'q3_a1',
      title: 'Q3-A1. 용변 후 청결 평가 (경로 A-1)',
      description: '상황: 인지 어려움 낮음 + 이동 어려움 낮음\n용변 후 청결(뒤처리)을 스스로 할 수 있나요?',
      type: 'single',
      options: [
    ...
    // 경로 B: 인지 어려움이 중등도 이상인 경우 (2~4점)
    q2_b: {
      id: 'q2_b',
      title: 'Q2-B. 화장실 이동 평가 (경로 B)',
      description: '화장실까지 스스로 이동하는 데 어려움이 있나요?',
      type: 'single',
      options: [
  ...
  results: {
    'B-A': {
      ...
      reason: '배뇨/배변 인지(Q1), 화장실 이동(Q2-A), 용변 후 청결(Q3-A1) 평가에서 모두 어려움이 낮음(0~1점)으로 판정되었습니다.'
    },
    'B-B': {
      id: 'B-B',
      ...
      reason: '배뇨/배변 인지 조절과 화장실 이동 능력은 양호하나, 용변 후 위생 뒤처리(Q3-A1) 단계에서 중간 정도 이상의 기능적 지장(2~4점)이 평가되었습니다.'
    },
    'B-C': {
      id: 'B-C',
      title: 'B-C. 화장실 이동 돕기',
      description: '배설 의사는 명확히 인지하고 뒤처리 능력도 갖추었으나, 보행 장애나 균형 저하로 인해 침대에서 화장실까지 물리적으로 안전하게 이동하는 데 어려움이 큽니다.',
      recommendation: '보행 보조기(워커), 휠체어, 또는 화장실 문턱 제거 및 안전 바 설치, 간이 이동변기 활용을 추천합니다.',
      reason: '배뇨/배변감 인지는 잘하고 뒤처리는 가능하지만, 화장실까지의 이동(Q2-A) 단계에서 낙상 위험 등 중등도 이상의 어려움(2~4점)을 겪고 있습니다.'
    },
    'B-D': {
      id: 'B-D',
      title: 'B-D. 침상 배변 또는 이동변기 이용',
      description: '배설감은 스스로 느끼지만, 화장실로의 이동과 용변 후 뒤처리를 독자적으로 수행하기 불가능하므로 침실 내에서 해결하는 보조 솔루션이 권장됩니다.',
      recommendation: '방 안에 배치할 수 있는 침상용 이동변기(Portal Commode) 또는 침상용 소형 기립보조/이승보조 보조기기를 고려하세요.',
      reason: '배뇨/배변감은 인지하지만, 이동(Q2-A)과 위생 뒤처리(Q3-A2) 모두에서 자립 수행이 불가능한 수준(2~4점)의 큰 제한이 존재합니다.'
    },
    'B-E': {
      id: 'B-E',
      title: 'B-E. 시간에 맞춘 배뇨·배변 프로그램 적용',
      description: '화장실로 걸어갈 수 있고 뒤처리도 할 수 있지만, 인지 기능 저하(치매 등)로 인해 배뇨/배변 시기를 알아차리지 못해 실금하는 위험이 높은 단계입니다.',
      recommendation: '스마트 배뇨 예측 센서(피부 부착형 또는 기저귀 센서)를 활용하여 정해진 시간마다 화장실로 동반 이동하는 체계적 훈련을 실시하세요.',
      reason: '배뇨/배변감 인지(Q1)에 중등도 이상의 장해(2~4점)가 있지만, 신체적 이동력과 뒤처리 기능은 보존되어 인지적 지원이 주로 필요합니다.'
    },
    'B-F': {
      id: 'B-F',
      title: 'B-F. 시간에 맞춘 배뇨·배변 프로그램 + 용변 후 처리 돕기',
      description: '배설 인지 능력이 떨어져 제때 화장실을 가기 어렵고, 신체적 관절 가동 범위 제한 등으로 용변 후 처리 능력마저 결여된 상태입니다.',
      recommendation: '주기적 알림 센서와 함께, 위생 위생비데 및 환자용 자동 세정/건조 장치를 보조로 구축하는 것을 고려하십시오.',
      reason: '배설 인지(Q1)와 위생 뒤처리(Q3-B1) 영역에서 동시에 독립성이 결여(2~4점)되었으나, 다행히 화장실까지의 이동은 비교적 가능한 상태입니다.'
    },
    'B-G': {
      id: 'B-G',
      ...
      recommendation: '야간 또는 특정 시간대에 탈부착이 가능한 세정식 자동배설처리로봇(Excretion Care Robot)의 간헐적 도입을 고려해 볼 수 있습니다.',
      reason: '배설 인지(Q1)와 이동 능력(Q2-B)이 모두 크게 손상되어 있으나, 신체 협조 또는 위생 청결(Q3-B2) 자체는 일정 수준 협조가 가능한 상황입니다.'
    },
    'B-H': {
      id: 'B-H',
      title: 'B-H. 자동 배설처리로봇 지속적 이용',
      description: '배설 인지, 이동 능력, 용변 후 뒤처리 자립도가 모두 상실된 전적인 와상(침상 고정) 상태의 대상자에게 가장 높은 등급의 간호를 제공하는 로봇형 배설 케어입니다.',
      recommendation: '침상에 누운 상태에서 소변과 대변을 감지하여 즉시 흡입, 온수 세정, 온풍 건조까지 전 과정을 사람 손 없이 자동 처리하는 흡입식 자동배설처리로봇을 24시간 연동 사용하기를 권장합니다.',
      reason: '배뇨/배변 인지(Q1), 화장실 이동(Q2-B), 용변 후 뒤처리 청결(Q3-B2)의 전 평가 영역에서 가장 중증인 극심한 어려움(2~4점) 상태를 보이고 있습니다.'
    }
  }
};
