...
export const resultDetails: Record<string, {
  ...
  'T-B': {
    deviceName: '가벼운 정도의 어려움이 있다면, • 이승보조장비 이용',
  ...
  'T-C': {
    deviceName: '전동형 기립보조리프트 (예: 히그, 업고플러스)',
  ...
  'T-F': {
    deviceName: '슬링 자동 삽입 (예: 맨엔텔 자세 변환형 전동의자)',
  ...
  'B-C': {
    deviceName: '화장실 이동 돕기 + 침상 배설 또는 이동 변기 이용',
  ...
  'B-D': {
    deviceName: '화장실 이동 돕기 + 침상 배설 또는 이동 변기 이용 / 용변 후 처리 돕기 추가 또는 자동 배설처리로봇 간헐적 이용',
  ...
  'B-E': {
    deviceName: '시간에 맞춘 배뇨훈련 또는 배변 프로그램 적용 / 용변 후 처리 돕기 추가',
  ...
  'B-F': {
    deviceName: '시간에 맞춘 배뇨훈련 또는 배변 프로그램 적용',
  ...
  'B-H': {
    deviceName: '자동 배설처리로봇 간헐적 이용 / 흡인형 스마트 기저귀 로봇시스템 지속적 이용',
    image: '/images/excretion_robot.png',
  ...
  // 식사돌봄 결과 기기 상세
  'F-A': {
    deviceName: '가벼운 정도의 어려움이 있다면, • 특수식사도구 이용 (코 부위가 절린 컵, 경사 접시 등) • 수동형 팔 지지대 이용',
    image: '',
  ...
  'F-C': {
    deviceName: '부분 식사보조기기 이용 (예: 스프링 팔 지지대) / 수동/반자동 식사돌봄로봇 이용',
  ...
  'F-D': {
    deviceName: '구강 섭취 불가능 시, • 의료진의 지시에 따름. • 건강 영양 또는 완전 비경구영양 / 식사 시 찾은 사례와 기침 시, • 구강 섭취(물 포함) 중단 • 의료진과 상의',
    image: '',
...
export const toiletingNodes: Record<string, { x: number; y: number; label: string; isResult?: boolean; isLabel?: boolean; typeLabel: string }> = {
  // 배뇨감 레이블
  'lbl-start':  { x: 790, y: 0,    label: "배뇨감 인지 평가",                  isLabel: true, typeLabel: "" },
  // q1: 중앙
  q1:           { x: 770, y: 60,   label: "중간 이상 어려움 (2~4)이 있는가",  typeLabel: "" },
  // q1 아니오(왼쪽) / 예(오른쪽) → 화장실 이동 평가
  q2_a:         { x: 330, y: 480,  label: "중간 이상 어려움 (2~4)이 있는가", typeLabel: "화장실 이동 평가" },
  q2_b:         { x: 1210, y: 480, label: "중간 이상 어려움 (2~4)이 있는가", typeLabel: "화장실 이동 평가" },
  // q2 아니오/예 → 용변 후 청결 평가 (각 4개)
  q3_a1:        { x: 100, y: 820,  label: "중간 이상 어려움 (2~4)이 있는가", typeLabel: "용변 후 청결 평가" },
  q3_a2:        { x: 550, y: 820,  label: "중간 이상 어려움 (2~4)이 있는가", typeLabel: "용변 후 청결 평가" },
  ...
  'B-E':        { x: 920,  y: 1150, label: "시간에 맞춘 배뇨훈련 또는\n배변 프로그램 적용 /\n용변 후 처리 돕기 추가",                  isResult: true, typeLabel: "추천 결과" },
  'B-F':        { x: 1150, y: 1150, label: "시간에 맞춘 배뇨훈련 또는\n배변 프로그램 적용",                                            isResult: true, typeLabel: "추천 결과" },
  'B-G':        { x: 1380, y: 1150, label: "자동 배설처리로봇 간헐적 이용",                                                             isResult: true, typeLabel: "추천 결과" },
  'B-H':        { x: 1610, y: 1150, label: "자동 배설처리로봇 간헐적 이용 /\n흡인형 스마트 기저귀 로봇시스템 지속적 이용",              isResult: true, typeLabel: "추천 결과" },
};
...
export const toiletingEdges = [
  ...
  { from: 'q2_a',  to: 'q3_a2', label: "예",     condition: (ans: any) => ans['q2_a']  === 'yes' },
  { from: 'q2_b',  to: 'q3_b1', label: "아니오", condition: (ans: any) => ans['q2_b']  === 'no' },
  { from: 'q2_b',  to: 'q3_b2', label: "예",     condition: (ans: any) => ans['q2_b']  === 'yes' },
  { from: 'q3_a1', to: 'B-A',   label: "아니오", condition: (ans: any) => ans['q3_a1'] === 'no' },
  { from: 'q3_a1', to: 'B-B',   label: "예",     condition: (ans: any) => ans['q3_a1'] === 'yes' },
  { from: 'q3_a2', to: 'B-C',   label: "아니오", condition: (ans: any) => ans['q3_a2'] === 'no' },
  { from: 'q3_a2', to: 'B-D',   label: "예",     condition: (ans: any) => ans['q3_a2'] === 'yes' },
  { from: 'q3_b1', to: 'B-E',   label: "아니오", condition: (ans: any) => ans['q3_b1'] === 'no' },
  { from: 'q3_b1', to: 'B-F',   label: "예",     condition: (ans: any) => ans['q3_b1'] === 'yes' },
  { from: 'q3_b2', to: 'B-G',   label: "아니오", condition: (ans: any) => ans['q3_b2'] === 'no' },
...
export const optionDetails: Record<string, string> = {
  ...
  // 1-1. 배설 q1 (배뇨감 인지) 0-4 척도 상세
  'toileting_q1_0': '스스로 소변이나 대변이 마려운 것을 느끼고 화장실 이용 의사를 잘 표현하십니다.',
  'toileting_q1_1': '용변 신호를 인지하지만 가끔 타이밍을 놓쳐 가벼운 실수가 있는 상태입니다.',
  'toileting_q1_2': '치매나 인지저하 등으로 소대변 신호를 혼자 알아채기 어려워 실금이 잦습니다.',
  'toileting_q1_3': '배설 욕구를 거의 인지하지 못하여 하루에 수차례 정기적으로 화장실을 안내해야 합니다.',
  'toileting_q1_4': '요의나 변의 인지가 아예 불가능하여 24시간 내내 실수가 발생하는 상태입니다.',

  // 1-2. 이승 q2 (하지 지력) 예/아니오 상세
  'q2_yes': '다리에 힘이 없어 본인의 힘으로 서서 몸무게를 버티지 못합니다. (Grade 0~III)',
  'q2_no': '보호자가 부축해 주면 잔존 다리 힘으로 서 있거나 버티실 수 있습니다. (Grade IV~V)',

  // 1-3. 이승 q4 (상체 조절) 예/아니오 상세
  'q4_yes': '앉아 있을 때 허리와 고개가 꺾이고, 보조 손잡이를 꽉 잡지 못합니다.',
  'q4_no': '앉은 상태에서 스스로 상체를 세우고, 리프트 앞 손잡이를 힘있게 잡을 수 있습니다.',

  // 2. 식사돌봄 (feeding) q1 (삼킴 가능여부) 예/아니오 상세
  'feeding_q1_yes': '음식을 삼키는 기능(삼킴 반사)이 양호하며, 입을 통해 식사를 안전히 드실 수 있습니다.',
  'feeding_q1_no': '삼킴 기능이 극도로 제한되어 사래가 심하게 들리거나 질식 우려로 입 식사가 전면 불가합니다.',

  // 2-2. 식사 q2 (먹기/마시기 기능) 0-4 척도 상세
  'feeding_q2_0': '아무런 도구나 타인의 도움 없이도 스스로 수저를 쥐고 온전히 식사하십니다.',
  'feeding_q2_1': '약간 서툴거나 숟가락질이 지체될 때가 있으나 자력으로 식사 해결이 가능합니다.',
  'feeding_q2_2': '숟가락을 쥐거나 밥그릇을 잡을 때 관절 통증이나 제한으로 가벼운 도구/부축 도움이 필요합니다.',
  'feeding_q2_3': '팔 떨림이나 쥐기 장애가 심해 식사 내내 반찬을 올려주거나 떠먹여주는 등의 완전 보조가 요구됩니다.',
  'feeding_q2_4': '스스로 밥을 먹으려는 손동작이나 신체적 기능이 전혀 유지되지 않는 상태입니다.',

  // 2-3. 식사 q3 (팔의 근력) 예/아니오 상세
  'feeding_q3_yes': '상지 마비나 극심한 약화로 식사할 때 팔을 어깨 높이로 전혀 들지 못합니다. (Grade 0~II)',
  'feeding_q3_no': '스스로 팔을 들어 올려 입 앞까지 유도하거나 중력에 대항할 수 있는 힘이 있습니다. (Grade III~V)',
