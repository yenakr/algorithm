...
export const transferNodes: Record<string, { x: number; y: number; label: string; isResult?: boolean; typeLabel: string }> = {
  ...
  q3: { x: 1060, y: 440, label: "슬링 적용방식 고려", typeLabel: "슬링 적용방식 고려" },
  q4: { x: 480, y: 440, label: "스스로 상체를 일으킬 수 없는가?", typeLabel: "스스로 상체를 일으킬 수 없는가?" },
  'T-B': { x: 150, y: 660, label: "가벼운 정도의 어려움이 있다면, • 이승보조장비 이용", isResult: true, typeLabel: "추천 결과" },
  'T-C': { x: 370, y: 660, label: "전동형 기립보조리프트 (예: 허그, 업고플러스)", isResult: true, typeLabel: "추천 결과" },
...
export const toiletingNodes: Record<string, { x: number; y: number; label: string; isResult?: boolean; typeLabel: string }> = {
  ...
  'B-A': { x: 0, y: 660, label: "도움분필요", isResult: true, typeLabel: "추천 결과" },
  'B-B': { x: 220, y: 660, label: "용변 후 처리 돕기 (비데 등)", isResult: true, typeLabel: "추천 결과" },
  'B-C': { x: 440, y: 660, label: "화장실 이동 돕기\n침상 배설 또는 이동 변기이용", isResult: true, typeLabel: "추천 결과" },
  'B-D': { x: 660, y: 660, label: "화장실 이동 돕기\n침상 배설 또는 이동 변기이용\n용변 후 처리 돕기 추가\n또는\n자동 배설처리로봇 간헐적 이용", isResult: true, typeLabel: "추천 결과" },
  'B-E': { x: 880, y: 660, label: "시간에 맞춘 배뇨훈련 또는 배변 프로그램 적용", isResult: true, typeLabel: "추천 결과" },
  'B-F': { x: 1100, y: 660, label: "시간에 맞춘 배뇨훈련 또는 배변 프로그램 적용\n용변 후 처리 돕기 추가", isResult: true, typeLabel: "추천 결과" },
  'B-G': { x: 1320, y: 660, label: "자동 배설처리로봇 간헐적 이용\n또는\n흡인형 스마트 기저귀 로봇시스템 지속적 이용", isResult: true, typeLabel: "추천 결과" },
  'B-H': { x: 1540, y: 660, label: "자동 배설처리로봇 지속적 이용", isResult: true, typeLabel: "추천 결과" },
};
...
export const feedingNodes: Record<string, { x: number; y: number; label: string; isResult?: boolean; typeLabel: string }> = {
  q1: { x: 570, y: 0, label: "구강섭취가 가능한가", typeLabel: "삼킴 기능 평가" },
