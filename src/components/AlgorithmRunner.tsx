...
const resultDetails: Record<string, {
  ...
  'F-A': {
    ...
    precautions: ['일정한 주기로 식사 시 사래 들림이나 손목 약화 징후를 부드럽게 관찰하세요.', '식사 자세가 지나치게 구부정해지지 않도록 가볍게 안내해 주는 것이 좋습니다.'],
    environment: '일반 식탁 및 주거 환경',
    reason: '식사 관련 인지, 삼킴 및 손동작 자립도가 모두 양호하여 현재 기기 지원이나 로봇 보조가 불필요합니다.'
  },
  'F-B': {
    deviceName: '기능성 보조 식기 및 그립 지원 도구',
    image: '/images/transfer_board.png', // Fallback image representation
    whenToUse: '삼킴은 문제없으나 손떨림이나 가벼운 관절 약화로 수저 조작에 가벼운 어려움이 있는 분',
    pros: ['식사 시 음식을 흘리지 않아 대상자가 독립적으로 스스로 식사 유지 가능', '손목 관절과 손가락 잡는 기능의 잔존 발달 효과'],
    precautions: ['자이로 스푼의 경우 충전 상태를 수시로 체크해 주십시오.', '식기 매트가 식탁에서 잘 고정되도록 바닥의 이물질을 닦아주세요.'],
    environment: '일반 주거 및 식당 식탁 환경',
    reason: '삼킴(연하) 기능은 양호하나 상지 숟가락 조절 시 경증/중등도의 어려움을 보이므로 그립 식기가 추천됩니다.'
  },
  'F-C': {
    deviceName: '전동 식사보조 로봇',
    image: '/images/smart_diaper_robot.png', // Fallback image representation
    whenToUse: '삼킴에는 무리가 없으나 양팔의 완전 마비 등으로 스스로 숟가락을 집는 행위가 불가능한 고령자/환자',
    pros: ['전동 로봇팔이 반찬과 밥을 떠서 안전하게 고령자의 입 앞까지 배달', '보호자가 매번 직접 급식하지 않고 대상자가 자기 페이스대로 주도적 식사 가능'],
    precautions: ['사용자의 머리와 고개는 조절할 수 있어야 안전하게 밥을 입에 받아넘길 수 있습니다.', '초기에 기기 속도와 버튼 또는 센서 입력에 충분한 적응 기간이 필요합니다.'],
    environment: '평평하고 넓은 넓이의 식사 보조 전용 테이블',
    reason: '숟가락질이 전혀 힘들어 불가한 중증 상지 지체 상태이지만, 고개를 내밀어 삼킬 수는 있어 전동 식사로봇이 매우 적절합니다.'
  },
  'F-D': {
    deviceName: '연하 보조식 제공 및 보호자 밀착 급식 보조',
    image: '/images/toilet_lift.png', // Fallback image representation
    whenToUse: '삼킴 장애가 심하여 사래 들림이나 기도 흡인, 질식의 우려가 높아 전문적인 관찰이 절실히 필요한 상태',
    pros: ['질식 및 흡인성 폐렴과 같은 긴급 생명 위협 위험을 완전 차단', '환자의 호흡 및 저작 속도에 맞춰 음식 공급을 실시간으로 완만하게 조절 가능'],
    precautions: ['기도 흡인을 예방하기 위해 식사 후 최소 30분간 상체를 30~60도 세운 각도로 앉아있게 하십시오.', '음식을 갈아 점도 증진제와 함께 겔 또는 죽 형태로 제공해야 합니다.'],
    environment: '침실 또는 등받이 조절형 안전 의자 공간',
    reason: '연하 곤란 점수가 높아 기도 폐쇄 우려가 높으므로, 기계에 의존하기보다 전문 보호자가 호흡에 맞추어 급식을 전담하는 것이 최선입니다.'
  },
  'F-E': {
    deviceName: '인지 환기 보조 장치 및 보호자 지도 관리',
    image: '/images/excretion_robot.png', // Fallback image representation
    whenToUse: '식사하는 행위 자체를 지속하지 못하고 멍하니 있거나 치매 등으로 인지 자극 지연이 있는 고령자',
    pros: ['스피커나 친밀한 가이드를 통해 식사 단계를 주기적으로 환기해 식사 속도 유지', '보호자가 매번 식사 시간 내내 반복해 일깨워 주는 심리 피로 완화'],
    precautions: ['환자가 안내음에 놀라거나 불안해하지 않도록 친근하고 익숙한 목소리로 음성을 조절하십시오.', '알림 센서 오작동으로 다 씹기도 전에 추가 안내가 나가지 않게 조치하세요.'],
    environment: '안정감 있고 소음이 정돈된 식사 환경',
    reason: '삼킴과 신체 거동은 비교적 가능하나 인지 지체로 행동 조절 능력이 저하되어 있으므로 인지적 주의 환기 장치가 필요합니다.'
  }
};
                          ...
                          isResult
                            ? `rounded-2xl border-2 ${
                                isHighlightedResult
                                  ...
                                  : isActive
                                    ? 'border-teal-500 bg-teal-50 text-teal-950 shadow-md ring-4 ring-teal-500/20 scale-[1.02] z-20 cursor-default'
                      ...
                      >
                        <div className="flex-1 flex flex-col justify-between gap-3">
                          ...
                          {!isResult && outgoingEdges.length > 0 && (
                            <div 
                              className="pt-2 border-t border-slate-100 mt-auto flex flex-row gap-2 w-full flex-nowrap"
                            >
                              {outgoingEdges.map((edge, eIdx) => {
                                ...
                                return (
                                  <button
                                    ...
                                    className={`px-3 py-2 rounded-xl text-sm sm:text-base font-extrabold transition-all border text-center leading-tight whitespace-nowrap cursor-pointer flex items-center justify-center min-h-[42px] flex-1 ${
                                      isBranchSelected
