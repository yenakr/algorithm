...
// Data layers
import { algorithmsData, robotTypeInfo, CaseStudy } from '@/data';

// Component imports
import AlgorithmRunner from '@/components/AlgorithmRunner';

interface AlgorithmClientPageProps {
  algoId: string;
  quizzes: CaseStudy[];
}
...
export default function AlgorithmClientPage({ algoId, quizzes }: AlgorithmClientPageProps) {
  ...
  // Quiz state
  const [sessionQuizzes, setSessionQuizzes] = useState<CaseStudy[]>([]);
  ...
  const generateSessionQuizzes = () => {
    const rawQuizzes = quizzes || [];
    if (rawQuizzes.length > 0) {
      const categories: Record<string, CaseStudy[]> = { A: [], B: [], C: [], D: [] };
  ...
  useEffect(() => {
    const sessionQuizzesList = generateSessionQuizzes();
    setSessionQuizzes(sessionQuizzesList);
    setCategoryScores({ A: 0, B: 0, C: 0, D: 0 });
    setQuizResults(Array(sessionQuizzesList.length).fill(null));
  }, [algoId, quizzes]);
    ...
    <div className="flex-1 bg-slate-50 pb-20">
      ...
      <div className="sticky top-0 z-50 bg-white/95 border-b border-slate-200/80 shadow-sm backdrop-blur-md py-2.5">
        <div className="max-w-[1400px] mx-auto px-4 flex items-center justify-between gap-4">
          ...
          <Link href="/" className="inline-flex items-center gap-1.5 text-xs font-extrabold text-slate-600 hover:text-slate-900 transition-colors shrink-0">
          ...
          <div className="flex-1 flex items-center justify-start md:justify-center gap-2 overflow-x-auto no-scrollbar scroll-smooth">
            ...
            <div className="flex items-center gap-1.5 shrink-0">
              {[
                ...
                { id: 'step-learning', name: '사용자용 (쉬운 버전)' },
                      ...
                      {devicesList[deviceIndex]?.situations.map((sit, i) => (
                        <li key={i}>
                          {cleanInternalCodes(sit)
                            ...
                            .split('|').map((chunk, i) => i % 2 === 1 ? <span key={i} className="text-slate-900 font-black bg-yellow-50 px-1 rounded">{chunk}</span> : chunk)}
                        </li>
                      ))}
                    </ul>
                  </div>
                ...
                <div className="space-y-6 animate-fade-in">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3.5">
                    {sessionQuizzes[quizIndex].options.map((option: string, idx: number) => {
                      ...
                      let optionStyle = 'border-slate-200 hover:bg-slate-50 text-slate-700 bg-white hover:border-slate-300';
                      if (isSelected) {
                    ...
                    <div className="bg-slate-50 border border-slate-200 rounded-2xl p-6 text-left space-y-6">
                      ...
                      <div className="bg-white border border-slate-200/80 rounded-xl p-5 space-y-3.5">
                        ...
                        {(() => {
                          ...
                          return (
                            <ul className="space-y-2 list-disc pl-4 text-xs sm:text-sm text-slate-600 font-semibold leading-relaxed">
        ...
        >
          <div className="relative max-w-5xl w-full max-h-[90vh] bg-white rounded-2xl p-4 overflow-auto flex flex-col items-center gap-4" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center w-full border-b pb-2">
              ...
              <button
                onClick={() => setIsModalOpen(false)}
                className="text-slate-500 hover:text-slate-800 font-bold text-sm bg-slate-100 hover:bg-slate-200 px-3 py-1.5 rounded-lg transition-colors cursor-pointer"
              >
                닫기
              </button>
            </div>
